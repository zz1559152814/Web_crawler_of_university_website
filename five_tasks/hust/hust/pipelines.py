...
class HustPipeline(object):
    ...
    def _conditional_insert(self,tx,items):  
        sql = r'insert into news(id ,title ,author,time ,resource ,click,module,content) values (%s,%s,%s,%s,%s,%s,%s,%s)'
        #print items['time'],items['resource'],items['click'],items['module'],items['content']
        param = (items['id'], items['title'],items['author'], items['time'] ,items['resource'],items['click'],items['module'],items['content'])
